...
test_that("Correct conversion for a single cohortId using default analysis ID", {
  # For a single value, e.g., 1, the expected value is (1 * 1000) + 150
  cohortId <- 1
  expected <- (cohortId * 1000) + 150
  result <- Kala::convertCohortIdToCovariateId(cohortId)
...
test_that("Correct conversion for a vector of cohortIds using default analysis ID", {
  ...
  result <- Kala::convertCohortIdToCovariateId(cohortIds)
  expect_equal(result, expected)
})
...
test_that("Conversion works with a custom cohortCovariateAnalysisId", {
  ...
  result <- Kala::convertCohortIdToCovariateId(cohortIds, cohortCovariateAnalysisId = customId)
  expect_equal(result, expected)
})
